...
namespace Nino.Shared.Mgr
{
    public static class CompressMgr
    {
        ...
        /// <param name="outputLength"></param>
        /// <returns></returns>
        public static ExtensibleBuffer<byte> Decompress(byte[] data,int index,int count, out int outputLength)
        {
#if !UNITY_2017_1_OR_NEWER
            ExtensibleBuffer<byte> buffer = ObjectPool<ExtensibleBuffer<byte>>.Request();
            var dt = DecompressOnNative(data,index,count);
            buffer.CopyFrom(dt, index, 0, count);
            outputLength = dt.Length;
            return buffer;
#endif
            lock (DecompressedLock)
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        private static byte[] DecompressOnNative(byte[] data,int index,int count)
        {
            using (var compressedStream = new MemoryStream(data,index,count))
            using (var zipStream = new System.IO.Compression.DeflateStream(compressedStream, CompressionMode.Decompress))
            using (var resultStream = new MemoryStream())
            {
                zipStream.CopyTo(resultStream);
