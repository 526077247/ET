...
namespace Nino.Serialization
{
	...
	public static class Deserializer
	{
		...
		public static object DeserializeWithoutGenerated(Type type,byte[] data, Encoding encoding = null)
		{
			...
			val = Activator.CreateInstance(type);
#endif
			return Deserialize(type, val, data, encoding ?? DefaultEncoding);
		}
		/// <summary>
		/// Deserialize a NinoSerialize object
		/// </summary>
		...
		public static T Deserialize<T>(byte[] data, Encoding encoding = null)
		{
			...
			if(t is ILRuntime.Reflection.ILRuntimeType)
				val = ILRuntimeResolver.CreateInstance(t);
			else
				val = Activator.CreateInstance(t);
#else
			val = Activator.CreateInstance(t);
#endif
			return (T)Deserialize(t, val, data, encoding ?? DefaultEncoding);
		}
